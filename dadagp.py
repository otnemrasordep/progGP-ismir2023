...
def tokens2guitarpro(all_tokens, verbose=False):
    ...
    for i,instrument in enumerate(track_numbering):
        ...
        if(instrument=="drums"):
            ...
            new_track.color = gp.Color(r=100, g=100, b=250)
            new_track.name = "Drums"
        elif(instrument=="bass"):
            new_track.channel.instrument = 34 # Electric Bass (pick) 
            new_track.color = gp.Color(r=215, g=215, b=100)
            new_track.name = "Bass"
            ...
            new_track.color = gp.Color(r=120, g=230, b=120)
...
def dadagp_batch_encode(input_dir, output_dir, artist_token):
    files = glob.glob(input_dir + '/**/*.gp5', recursive=True)
    count = 0
    bad_files = []
    for f in files:
        output_name = os.path.join(output_dir, f.split('/')[2].split('.')[0] + '.' + 'txt')
        print(f)
        dadagp_encode(f,output_name, artist_token)
    #    try:
    #        dadagp_encode(f,output_name, artist_token)
    #    except: 
    #        count += 1
    ...
    #print('{} files messed up in encoding'.format(count))
    #for f in bad_files:
    #    print(f)
...
def dadagp_batch_decode(input_dir, output_dir):
    files = glob.glob(input_dir + '/**/*.txt', recursive=True)
    count = 0
    for id, f in enumerate(files):
        output_name = os.path.join(output_dir, f.split('/')[-1].split('.')[0] + '.gp5')
        print(f)
        dadagp_decode(f,output_name)
...
def dadaGP(operation, input_file, output_file, epoch=205):
...
"""
    if operation in ["encode", "decode", "batch_encode", "batch_decode"]:
        ...
        if(operation=="encode"):
            dadagp_encode(input_file, output_file, 'artist:dream_theater')
        elif(operation=="batch_encode"):
            dadagp_batch_encode(input_file, output_file, 'artist:dream_theater')
        elif(operation=="batch_decode"):
            dadagp_batch_decode(input_file, output_file)
        else:
